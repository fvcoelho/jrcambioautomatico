      ...
      <div className={`h-56 bg-gradient-to-br ${gradient} flex items-center justify-center relative overflow-hidden`}>
        {images.length > 0 ? (
              ...
              <>
                ...
                <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 flex space-x-2">
                  {images.map((_, index) => (
                    <button
                      key={index}
                      className={`carousel-control w-2 h-2 rounded-full transition-all duration-300 ${index === carousel.currentIndex ? 'bg-white' : 'bg-white/50 hover:bg-white/70'
                        }`}
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent flex items-center justify-center">
            <div className="text-white/70 text-sm uppercase tracking-widest">Sem mídia</div>
          </div>
        )}
        ...
        <div className="p-8">
                  ...
                  <>
                    ...
                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-3">
                      {images.map((_, index) => (
                        <button
                          key={index}
                          className={`w-3 h-3 rounded-full transition-all duration-300 ${index === carousel.currentIndex ? 'bg-white' : 'bg-white/50 hover:bg-white/70'
                            }`}
          ...
          {images.length > 1 && (
            <div className="flex space-x-2 mb-8 overflow-x-auto">
              {images.map((image, index) => (
                <button
                  key={image.id}
                  className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all duration-300 relative ${index === carousel.currentIndex ? 'border-green-500' : 'border-transparent hover:border-gray-300'
                    }`}
                  onClick={() => carousel.goToImage(index)}
                >
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            <div>
              ...
              <div className="space-y-2">
                <p className="text-green-600 font-semibold">Local: {project.location || 'Localização não informada'}</p>
                {project.completedAt && (
          ...
          <div className="flex gap-4">
            <a
              ...
              className="bg-gradient-to-r from-green-600 to-green-700 text-white px-8 py-4 rounded-xl font-semibold hover:from-green-700 hover:to-green-800 transition-all duration-300 flex-1 text-center font-montserrat shadow-lg flex items-center justify-center gap-2"
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="font-montserrat text-gray-500">Preparando trabalhos recentes...</div>
        </div>
      </div>
    )
  }
