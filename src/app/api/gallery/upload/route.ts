...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let blob: { url: string; pathname: string; downloadUrl?: string }

    if (STORAGE_TYPE === 'local') {
      console.log('📤 Uploading to local storage...')

      // Upload to local filesystem
      const localResult = await localStorageService.uploadFile(file, 'gallery')
      blob = {
        url: localResult.url,
    ...
    let displayOrder = 0
    if (displayOrderStr) {
      displayOrder = parseInt(displayOrderStr)
    } else {
      ...
      displayOrder = (maxOrderImage?.displayOrder ?? -1) + 1
    }
    ...
    // Test blob URL accessibility (skip for local storage as it's served by Next.js)
    if (STORAGE_TYPE !== 'local') {
      try {
